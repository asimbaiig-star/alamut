...
| F39 | Supabase's built-in auth email service is capped at **2 emails/hour** (per Supabase's own production checklist, which calls it unsuitable for production) — a third signup within the same hour never receives its link and cannot sign in. Custom SMTP raises the cap to **30 new users/hour**. Resolution path (2026-08-09): **Gmail SMTP via an app password** needs no domain and costs nothing, so it beats both the built-in service and Resend (whose free tier requires a domain you own — `*.vercel.app` can't be DNS-verified). Domain + Resend remains the eventual upgrade. |
